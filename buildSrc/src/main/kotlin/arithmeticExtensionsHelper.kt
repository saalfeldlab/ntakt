...
fun GenerateCode.arithmeticExtensionHelper() {
//    println("generating arithmetic extensions helper")
    FileSpec.builder(packageName, "ArtithmeticExtensionsHelper")
        .makeArithmeticBiConverters()
        .makeArithmeticConverterObject()
        .build().writeTo(targetDir.asFile)
}
...
private fun FileSpec.Builder.makeArithmeticConverterObject(): FileSpec.Builder {
    ...
    val codeBlock = CodeBlock
        .builder()
        .addStatement("return·when·(operator.lowercase())·{")
        .indent()
...
private val String.converterName get() = "BiConverter${capitalized()}"
...
private val arithmetics.Operator.converterClassNameStarProducer get() = operatorStarMapping[name.lowercase()]!!
