...
fun GenerateCode.converterExtensions() {
    for ((`as`, name) in getTypeFileMapping("Converter")) {
//        println("generating converter extensions for $`as` ($name)")
        generateConverterExtensions(`as`, name.first).writeTo(targetDir.asFile)
    }
}
...
fun generateConverterExtensions(`as`: String, fileName: String): FileSpec {
...
private fun generateIntegerTypeToRealTypeConversionExtensions(container: ClassName) = realAndIntegerTypes.map { generateIntegerTypeToRealTypeConversionExtensions("as${it.key.capitalized()}", container, it.value) }
...
private fun generateRealTypeToRealTypeConversionExtensions(container: ClassName) = realAndIntegerTypes.map { generateRealTypeToRealTypeConversionExtensions("as${it.key.capitalized()}", container, it.value) }
...
private fun generateIntegerTypeWildcardToRealTypeConversionExtensions(container: ClassName) = realAndIntegerTypes.map { generateIntegerTypeWildcardToRealTypeConversionExtensions("as${it.key.capitalized()}", container, it.value) }
...
private fun generateRealTypeWildcardToRealTypeConversionExtensions(container: ClassName) = realAndIntegerTypes.map { generateRealTypeWildcardToRealTypeConversionExtensions("as${it.key.capitalized()}", container, it.value) }
...
private fun generateRealTypeConversionExtensionsFromWildcard(container: ClassName) = generateGenericTypeConversionExtensionFromWildcard(container, RealType::class.asTypeName(), "realDouble", "setReal")

private fun generateIntegerTypeConversionExtensionsFromWildcard(container: ClassName) = generateGenericTypeConversionExtensionFromWildcard(container, IntegerType::class.asTypeName(), "integerLong", "setInteger")

private fun generateComplexTypeConversionExtensionsFromWildcard(container: ClassName) = generateGenericTypeConversionExtensionFromWildcard(container, ComplexType::class.asTypeName(), "realDouble" to "setReal", "imaginaryDouble" to "setImaginary")
    ...
    for ((ct, name) in arrayOf(ComplexDoubleType::class to "Double", ComplexFloatType::class to "Float")) {
        for (part in arrayOf("real", "imaginary")) {
            typedFuncSpecBuilder(part, container.parameterizedBy(ct.asTypeName()))
                .addAnnotation(AnnotationSpec.builder(JvmName::class).addMember("name = %S", "complex$name${part.capitalized()}").build())
    ...
    for ((ct, type) in arrayOf(ComplexDoubleType::class to DoubleType::class, ComplexFloatType::class to FloatType::class)) {
        ...
        for (part in arrayOf("real", "imaginary")) {
            typedFuncSpecBuilder("${part}ReadOnly", container.parameterizedBy(ct.asTypeName()))
                .addAnnotation(AnnotationSpec.builder(JvmName::class).addMember("name = %S", "complex$name${part.capitalized()}ReadOnly").build())
...
private fun FileSpec.Builder.addComplexPhasePowerConverters(container: ClassName): FileSpec.Builder {
    for (part in arrayOf("phase", "power")) {
        for ((ct, type) in arrayOf(ComplexDoubleType::class to DoubleType::class, ComplexFloatType::class to FloatType::class)) {
            val name = type.simpleName!!.replace("Type", "")
            typedFuncSpecBuilder("${part}ReadOnly", container.parameterizedBy(ct.asTypeName()))
                .addAnnotation(AnnotationSpec.builder(JvmName::class).addMember("name = %S", "complex${name.capitalized()}${part.capitalized()}ReadOnly").build())
                .addStatement("return convert(%T())·{·s,·t·->·t.setReal(s.$part$name)·}", type)
                .build()
                .let { addFunction(it) }
        }
